...
// Оставляем динамический рендеринг
...
export default async function ProfilePage() {
  try {
    ...
    if (!user) {
      redirect("/login");
      return null; // Добавляем return для TypeScript
    }
    ...
    // Собираем статистику пользователя с обработкой ошибок
    const [betStats] = await db
      ...
      .catch(() => [{ total: 0, wins: 0, wagered: 0, won: 0 }]);
    ...
    // Рассчитываем статистику с проверками на undefined
          ...
          <div className="relative flex flex-col sm:flex-row items-center sm:items-start gap-6">
            {user.avatarUrl ? (
              <img
                ...
                onError={(e) => {
                  e.currentTarget.src = "https://via.placeholder.com/96x96/8577f0/ffffff?text=" + label[0]?.toUpperCase();
                }}
              />
            ) : (
    ...
    return (
      <div className="max-w-5xl mx-auto px-5 py-8">
        <div className="gash-card p-6 text-center">
          <h2 className="text-xl font-bold mb-4">Ошибка загрузки профиля</h2>
          <p className="text-gray-400">Пожалуйста, попробуйте позже</p>
          <button
            onClick={() => window.location.reload()}
            className="gash-btn mt-4"
          >
            Обновить страницу
          </button>
        </div>
